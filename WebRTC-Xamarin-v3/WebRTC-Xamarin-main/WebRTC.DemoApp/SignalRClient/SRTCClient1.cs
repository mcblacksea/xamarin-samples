...
{
    public class SRTCClient1 : IRTCClient<RoomConnectionParameters>
    {
        ...
        private const string TAG = nameof(SRTCClient1);
        ...
        public SRTCClient1(ISignalingEvents<SignalingParameters> _signalingEvents, ILogger _logger = null)
        {
            signalingEvents = _signalingEvents;
            executor = ExecutorServiceFactory.CreateExecutorService(nameof(SRTCClient1));
            logger = _logger ?? new ConsoleLogger();

            State = Abstraction.ConnectionState.New;
        ...
        private async void ConnectToHubRoom()
        {
            try
            {
                ...
                //await App1.HubConnection.StartAsync();
                var str = "";
                if (roomConnectionParameters.IsInitator)
                {
                    str = await App1.HubProxy.Invoke<string>("StartRoomParametersAsync",
                    "huy");
                }
                else
                {
                    str = await App1.HubProxy.Invoke<string>("GetRoomParametersAsync",
        ...
        private void DisconnectFromHubRoom()
        {
            ...
            if (App1.HubConnection.State == Microsoft.AspNet.SignalR.Client.ConnectionState.Connected)
            {
        ...
        private void ReportError(string _errorMessage)
        {
            ...
            executor.Execute(() =>
            {
                if (State == Abstraction.ConnectionState.Error)
                    return;
                State = Abstraction.ConnectionState.Error;
        ...
        public Abstraction.ConnectionState State { get; private set; }
        ...
        public void SendOfferSdp(SessionDescription _sessionDescriotion)
        {
            ...
            {
                if (App1.HubConnection.State != Microsoft.AspNet.SignalR.Client.ConnectionState.Connected)//(State != ConnectionState.Connected)
                {
                ...
                var isOfferSent = await App1.HubProxy.Invoke<string>("SendOfferMessage", json);
        ...
        public void SendLocalIceCandidate(IceCandidate _candidate)
        {
            executor.Execute(async () =>
            {
                ...
                if (isInitiator)
                {
                    ...
                    var isIceSent = await App1.HubProxy.Invoke<string>("SendLocalIceCandidate", App1.HubConnection.ConnectionId, json);
        ...
        public void SendLocalIceCandidateRemovals(IceCandidate[] _candidates)
        {
            executor.Execute(async () =>
            {
                ...
                if (isInitiator)
                {
                    ...
                    var isIceRemoved = await App1.HubProxy.Invoke<string>("RemoveIceCandidates", json);
        ...
        public void SendAnswerSdp(SessionDescription _sessionDescription)
        {
            executor.Execute(async () =>
            {
                ...
                var isAnswerSent = await App1.HubProxy.Invoke<string>("SendAnswerMessage", json);
            });
        }
